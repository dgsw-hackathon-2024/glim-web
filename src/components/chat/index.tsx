...
const ChatBot = () => {
  const { ...chat } = useChat();
  const { handleChatbotClick } = useChatbotButton();
  return (
    <>
      <S.Container>
        <S.Main>
          ...
          <S.SendWrap>
            <S.MessageInput value={chat.chat} onChange={chat.handleUserChat} placeholder="질문을 입력해주세요!" />
            {chat.isLoading ? (
              <img src={SendLoading} />
            ) : (
              <img src={Send} alt="" onClick={chat.sendChat} style={{ cursor: "pointer" }} />
            )}
          </S.SendWrap>
        </S.Main>
